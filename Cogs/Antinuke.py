...
class Antinuke(commands.Cog):
    ...
    async def on_member_ban(self, guild, member):
      with open('whitelisted.json') as f:
            ...
            logs = logs[0]
            if logs.user.id == 806519089144135720 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
              pass
            else:
    ...
    async def on_sticker_create(self, sticker):
            ...
            await logs.user.ban(reason=f"ViRTUCY SECURITY | Anti Sticker Create")
    ...
    async def on_sticker_update(self, sticker):
            ...
            logs = logs[0]
            await logs.user.ban(reason=f"ViRTUCY SECURITY | Anti Update Create")
    ...
    async def on_sticker_delete(self, sticker):
            ...
            logs = logs[0]
            await logs.user.ban(reason=f"ViRTUCY SECURITY | Anti Sticker Update")      
    ...
    async def on_member_remove(self, member, guild):
      with open('whitelisted.json') as f:
            ...
            if logs.user.id == 806519089144135720 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
             ...
             await logs.user.ban(reason=f"ViRTUCY SECURITY | Anti Kick")
            ...
            if logs.user.id == 806519089144135720 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
             ...
             await logs.user.ban(reason=f"ViRTUCY SECURITY | Anti Prune")
    ...
    async def on_guild_channel_create(self, channel):
      with open('whitelisted.json') as f:
            ...
            logs = logs[0]
            if logs.user.id == 806519089144135720 or logs.user.id == 806519089144135720 or logs.user.id == guild.owner.id or str(logs.user.id) in whitelisted[str(guild.id)]:
              pass
            else:
    ...
    async def on_member_unban(self, guild, member):
      with open('whitelisted.json') as f:
            ...
            logs = logs[0]
            if logs.user.id == 806519089144135720 or logs.user.id == 806519089144135720 or str(logs.user.id) in whitelisted[str(guild.id)]:
              pass
            else:
